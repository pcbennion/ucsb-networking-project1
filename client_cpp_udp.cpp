...
int main(int argc, char *argv[])
{
    ...
    // send username
    writeSocket(sockfd, 0x01, username, (struct sockaddr*) &serv_addr, servlen);

    // recieve identifier and welcome message
    string identifier, message;
    char type = 0x00;
    readSocket(sockfd, type, identifier, (struct sockaddr*) &serv_addr, servlen);
    if(type != 0x02) // catch bad server init
        error("Invalid server initialization. Terminating.\n");
    readSocket(sockfd, type, message, (struct sockaddr*) &serv_addr, servlen);
    ...
    while(!exit && !cin.eof())
    {
        ...
        cin>>line;
        getline(cin, dummy);//remove pesky \n leftover from fetching string from cin
        ...
        switch(swInt)
        {
            ...
            case 1:
                ...
                getline(cin, line);
                writeSocket(sockfd, 0x04, identifier+"\n"+line, (struct sockaddr*) &serv_addr, servlen);
                break;

            // send PRINTALL command to the server, then recieve/print full message board
            case 2:
                writeSocket(sockfd, 0x03, "", (struct sockaddr*) &serv_addr, servlen);
                readSocket(sockfd, type, line, (struct sockaddr*) &serv_addr, servlen);
                cout<<line<<endl;
                break;

            // send EXIT command to the server, then exit loop
            case 3:
                writeSocket(sockfd, 0x06, "", (struct sockaddr*) &serv_addr, servlen);
                exit = 1;
                break;
